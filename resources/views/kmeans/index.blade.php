...
function get_jarak_centroid($pusat_centroid = array(), $rows_data = array()){
    ...
    foreach($rows_data as $key => $val){
        foreach($pusat_centroid as $k => $v){
            $arr[$key][$k] = get_jarak($rows_ata[$key], $pusat_centroid[$k]);
        }
    }
    return $arr;
}
...
<?php 
print_r($rows_penjualan);

if(!isset($rows_penjualan)):?>
<p>Data Penjualan tidak ada !!!!!!</p>
    <?php else:?>
<?php if(!empty($_GET['tanggal_awal'])):?>
    <div id="hasil" class="panel panel-default">
        ...
        <div class="panel-body">        
          ...
          <?php $kode_bar[$row->kode_alternatif]=1;?>
          <tr>
            <td>{{$no++ }}</td>
            <td>{{ $row->tanggal }}</td>
